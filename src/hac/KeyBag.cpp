...
pie::hac::KeyBagInitializer::KeyBagInitializer(bool isDev, const tc::Optional<tc::io::Path>& keyfile_path, const tc::Optional<tc::io::Path>& tik_path, const tc::Optional<tc::io::Path>& cert_path)
{
...
void pie::hac::KeyBagInitializer::importBaseKeyFile(const tc::io::Path& keyfile_path, bool isDev)
{
...
#define _SAVE_AES128KEY(key_name, dst) \
	...
	if (val.empty() == false) { \
		...
		if (dec_val.size() != tmp_aes128_key.size()) \
			throw tc::ArgumentException("pie::hac::KeyBagInitializer", "Key: \"" + key_name + "\" has incorrect length"); \
...
void pie::hac::KeyBagInitializer::importTitleKeyFile(const tc::io::Path& keyfile_path)
{

}

void pie::hac::KeyBagInitializer::importCertificateChain(const tc::io::Path& cert_path)
{
	// open cert file
...
void pie::hac::KeyBagInitializer::importTicket(const tc::io::Path& tik_path)
{
	// open cert file
...
void pie::hac::KeyBagInitializer::importKnownKeys(bool isDev)
{
